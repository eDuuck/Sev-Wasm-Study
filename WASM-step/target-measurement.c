...
int main(int argc, char **argv)
{
    ...
    while (steps < MAX_MEAS_POINTS) // && atomic_load(&should_run))
    {
        ...
        if (event_type == SEV_STEP_EVENT) // We have performed a step.
        {
            ...
            if (event_steps == -1)
            {
                printf("Weird step event, step %d.\n", steps);
                // FREE_AND_ACK_EV();
                // goto cleanup;
            }
            else if (event_steps > 0 && event_steps < MULTISTEP_LIMIT)
            {
                ...
                printf("Overstepped step event, step %d with %d instructions.\n", steps, event_steps);
                FREE_AND_ACK_EV();
                break;
            }
        }
    ...
    fprintf(log_file, "[%02d-%02d %02d:%02d:%02d] - [%02d-%02d %02d:%02d:%02d]: %s finished with %d recorded steps.\n",
